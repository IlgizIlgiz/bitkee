 ...
 * @returns {any}
 */
export function generate_addresses_batch(start_key_hex, count) {
    const ptr0 = passStringToWasm0(start_key_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.generate_addresses_batch(ptr0, len0, count);
    return ret;
}
 ...
 * @returns {string}
 */
export function generate_single_address(private_key_hex, compressed) {
    let deferred2_0;
    let deferred2_1;
    try {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('btc_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
